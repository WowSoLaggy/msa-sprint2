...
Было принято решение начать миграцию с сервиса бронирования по следующим причинам:
- Бронирование отелей является критическим функционалом для бизнеса для получения прибыли.
- Бронирование отелей является важным функционалом для пользователей.
- Сервис бронирования является центральным с архитектурной точки зрения, потому что имеет зависимости от всех остальных модулей.

Миграция первого модуля включает в себя вынос логики бронирования в отдельный микросервис, включая выделение собственной базы данных из монолитной.

Так же предполагается создание Kafka инфраструктуры, куда новый микросервис бронирования должен посылать сообщения о бронировании.
